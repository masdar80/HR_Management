...
class SuggestionInfo extends StatefulWidget {
  @override
  _SuggestionInfoState createState() => _SuggestionInfoState();
  final String sugges_id;
  SuggestionInfo(this.sugges_id);
}

class _SuggestionInfoState extends State<SuggestionInfo> {

  Setting conf = new Setting();
  List<SuggestionCompliance> feeds;
  ...
  Widget build(BuildContext context) {
    ...
    feeds = (
        ...
        LoadSettings.getSuggestList().where((i) => i.compliant_id==widget.sugges_id).toList()
            :LoadSettings.getSuggestManagerList().where((i) => i.compliant_id==widget.sugges_id).toList()
    );
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("feed_info".tr, style: ScreenUtils.geHeaderextstyle()),
        centerTitle: true,
