...
class SuggestionsList extends StatefulWidget {
  ...
  SuggestionsList(this.documentType);
}

class _SuggestionsListState extends State<SuggestionsList> {

  Setting conf = new Setting();
  List<SuggestionCompliance> suggestions,suggestionsManager;
  List<SuggestionCompliance> filteredList = [],filteredListManager = [];
  ...
  void initState() {
    ...
    suggestions = LoadSettings.getSuggestList();
    suggestionsManager = LoadSettings.getSuggestManagerList();
  }
  @override
  Widget build(BuildContext context) {

    AppConstants.FEED_ALL="YES";
    suggestions = LoadSettings.getSuggestList();
    ...
    listScreens = [
      ...
      suggestionTab(_getDataToShow(),filterBarWidget),
      suggestionTab(_getDataManagerToShow(),filterBarWidgetManager),
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            Padding(
              ...
              onPressed: () => Navigator.of(context).pushNamed('add_suggestion'),
            )),
          ...
          title: Text("sug_comp".tr,style:TextStyle(color: Colors.white,fontSize:6+ScreenUtils.GetHomePageFontSize())),
          centerTitle: true,
  ...
  Widget suggestionTab(List<SuggestionCompliance> list,FilterBarWidget  filterWidg)
  {
  ...
  List<SuggestionCompliance> _getDataToShow() {
    return filterEnabled ? filteredList : suggestions;
  }
  List<SuggestionCompliance> _getDataManagerToShow() {
    return filterEnabledManager ? filteredListManager : suggestionsManager;
  }
  ...
  _searchList(String searchQuery) {
    setState(() {
      ...
      filteredList = suggestions.where((element) {
        return _filterByName(element) && _filterByID(searchQuery.trim(), element);
      }).toList();
      filteredList.sort((a, b) => a.compliant_id.compareTo(b.compliant_id));
      filterEnabled = true;
  ...
  _searchManagerList(String searchQuery) {
    setState(() {
      ...
      filteredListManager = suggestionsManager.where((element) {
        return _filterByName(element) && _filterByID(searchQuery.trim(), element);
      }).toList();
      filteredListManager.sort((a, b) => a.compliant_id.compareTo(b.compliant_id));
  ...
  _filterByName(SuggestionCompliance element) {
    return widget.documentType.toLowerCase() == AppConstants.TYPE_ALL.toLowerCase() || element.compliant_id.toLowerCase() == widget.documentType.toLowerCase();
  }

  _filterByID(String query, SuggestionCompliance element) {
    return element.compliant_id.toLowerCase().contains(query.toLowerCase());
  }

  List<String> sortingHeaders = ["ID", "Employee Name","Type", "Date"];
  List<bool> sortArray = [false, false, false, false, false, false, false, false];

  List<FilterModel> filters = [
    FilterModel(caption: "ID", fieldName: "ComplimentID", checked: false, selectedCompareOption: "", isNumber: false, isDate: false),
    FilterModel(caption: "Employee Name", fieldName: "EmployeeName", checked: false, selectedCompareOption: "", isNumber: false, isDate: false),
    FilterModel(caption: "Type", fieldName: "ComplimentType", checked: false, selectedCompareOption: "", isNumber: false, isDate: false),
    FilterModel(caption: "Date", fieldName: "ComplimentDateTime", isDate: true, selectedCompareOption: "", isNumber: false, checked: false, fromDate: DateTime.now(), toDate: DateTime.now()),
  ];
  ...
  _sortData(int sortingIndex) {
    ...
    setState(() {
      ...
      if (sortingIndex == -1) {
        _getDataToShow().sort((a, b) => a.compliant_id.compareTo(b.compliant_id));
      } else {
  ...
  _sortDataManager(int sortingIndex) {
    ...
    setState(() {
      // Navigator.pop(context);
      if (sortingIndex == -1) {
        _getDataManagerToShow().sort((a, b) => a.compliant_id.compareTo(b.compliant_id));
      } else {
  ...
  _sortBy(SuggestionCompliance a, SuggestionCompliance b, int sortingIndex) {
    switch (sortingIndex) {
      case 0:
        return a.compliant_id.trim().compareTo(b.compliant_id.trim());
      case 1:
        return b.compliant_id.trim().compareTo(a.compliant_id.trim());
      ...
      case 4:
        return a.complain_suggestion.trim().compareTo(b.complain_suggestion.trim());
      case 5:
        return b.complain_suggestion.trim().compareTo(a.complain_suggestion.trim());

      case 6:
        return dateFormat.parse(a.date_time).compareTo(dateFormat.parse(b.date_time));
      case 7:
        return dateFormat.parse(b.date_time).compareTo(dateFormat.parse(a.date_time));
  ...
  _filterData() {
    ...
    if (enabledFilters.isNotEmpty) {
      enabledFilters.forEach((filterModel) {
        filteredList = (filteredList.isNotEmpty ? filteredList : suggestions).where((suggestion) {
  ...
  _filterDataManager() {
    ...
    if (enabledFilters.isNotEmpty) {
      enabledFilters.forEach((filterModel) {
        filteredListManager = ((filteredListManager!=null &&filteredListManager.isNotEmpty) ? filteredListManager : suggestionsManager).where((suggestion) {
          return filterModel.isDocumentTypeFilter
              ? FilterUtilities.filterByDocumentType(suggestion, filterModel.fieldName, filterModel.selectedCompareOption)
              : (_filterByName(suggestion) && filterModel.isDate
              ? FilterUtilities.filterByDate(suggestion, filterModel.fieldName, filterModel.selectedCompareOption, filterModel.fromDate, false, searchDate2: filterModel.toDate)
